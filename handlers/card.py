...
HELP_TEXT = (
    "🤖 *ЛФЛ Агент — Справка*\n\n"
    "📲 *Как открыть приложение:* нажми кнопку *Меню* (☰ слева от поля ввода) или кнопку "
    "«⚽ Открыть приложение» ниже. Всё внутри приложения — снизу 4 вкладки.\n\n"
    "🪪 *Карточка* — твоя анкета игрока\n"
    "• Создать по ссылке с *lfl.ru / afl.ru / f-league.ru* (статистика подтянется сама) или вручную\n"
    "• Фото, редактирование, удаление\n"
    "• «🔄 Обновить с сайта» — перечитать свежую статистику\n"
    "• «🔍 Ищу команду» и «📣 В поиск агентов» — чтобы тебя находили тренеры\n\n"
    "🔍 *Поиск* — свободные агенты\n"
    "• Фильтр по позиции, тап по игроку — полная карточка\n"
    "• «Написать» — связаться напрямую\n\n"
    "🏟 *Команды*\n"
    "• Список с фильтрами: лига → округ (для ЛФЛ) → позиция\n"
    "• «👥 Моя команда» — зарегистрировать, эмблема, редактирование\n"
    "• «📩 Подать заявку» — откликнуться, капитану придёт уведомление\n\n"
    "👤 *Профиль* — статус и быстрые действия\n\n"
    "📢 *Уведомления:* когда появляется подходящий игрок или команда, бот пришлёт уведомление автоматически.\n\n"
    "❓ Остались вопросы — нажми «✉️ Написать администратору» ниже."
)
